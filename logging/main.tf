  ...
  containers.conf: |
    ...
    <label @containers>
      ...
      <match **>
        ...
        log_group_name "#{ENV.fetch('CLUSTER_NAME')}-k8s/application"
  ...
  host.conf: |
    ...
    <label @hostlogs>
      ...
      <match host.**>
        ...
        region "#{ENV.fetch('REGION')}"
        log_group_name "#{ENV.fetch('CLUSTER_NAME')}-k8s/host"
        log_stream_name_key stream_name
        remove_log_stream_name_key true
        auto_create_stream true
        <buffer>
  ...
  systemd.conf: |
    ...
    <label @systemd>
      ...
      <match **>
        ...
        region "#{ENV.fetch('REGION')}"
        log_group_name "#{ENV.fetch('CLUSTER_NAME')}-k8s/dataplane"
        log_stream_name_key stream_name
        auto_create_stream true
        remove_log_stream_name_key true
        <buffer>
